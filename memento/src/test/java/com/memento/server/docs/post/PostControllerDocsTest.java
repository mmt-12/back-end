		...
		mockMvc.perform(
			...
			.andDo(document("post-read-test",
				...
				responseFields(
					...
					fieldWithPath("comments.emojis[].count").type(NUMBER).description("이모지 공감 갯수"),
					...
					fieldWithPath("comments.voices[].count").type(NUMBER).description("보이스 공감 갯수"),
					...
					fieldWithPath("comments.temporaryVoices[].count").type(NUMBER).description("버블 보이스 공감 갯수"),
